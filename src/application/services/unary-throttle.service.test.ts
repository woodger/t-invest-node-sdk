...
describe('Throttle', () => {
  ...
  test('divides the resolved limit equally between active participants', async () => {
  ...
  test('uses the current participant count for queued calls', async () => {
    let participantCount = 1;
    const throttle = new Throttle(() => participantCount);
    ...
    await withControlledThrottleTimers(async ({
      ...
      participantCount = 2;
      setCurrentTime(10_600);
      runNext();
      await waitingCall;

      assert.deepEqual(delays, [600, 1200]);

      setCurrentTime(11_800);
      runNext();
      await followingCall;
    });
  });

  test('does not retain a request when participant resolution fails', async () => {
    const resolutionError = new Error('participant resolution failed');
    let shouldFail = true;
    const throttle = new Throttle(() => {
      if (shouldFail) {
