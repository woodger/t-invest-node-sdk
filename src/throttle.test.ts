...
test('resolveLimit returns the configured limit for a matching path', () => {
...
test('resolveLimit returns undefined for an unknown path', () => {
  const throttle = new Throttle({
    KnownService: 100
  });
...
test('resolveLimit prefers the most specific matching key for overlapping routes', () => {
  const throttle = new Throttle({
    OrdersService: 100,
...
test('resolveLimit does not depend on object key order for overlapping routes', () => {
  const throttle = new Throttle({
    '/tinkoff.public.invest.api.contract.v1.OrdersService/GetOrders': 200,
    OrdersService: 100
  });
