...
- Инструкции по генерации proto и публикации релиза перенесены из README в GitHub-only раздел `docs/development/`; release workflow больше не содержит конкретную версию пакета.
- `@bufbuild/protobuf` обновлён до 2.15.0, `@types/node` — до 26.5.1, `oxlint` — до 1.83.0, `ts-proto` — до 2.12.3. После перегенерации 12 proto-модулей изменились только заголовки с версией generator-а; generated API остался прежним.
...
- В README добавлены бейджи версии npm-пакета, поддерживаемой версии Node.js,
  наличия TypeScript declarations и лицензии.

### Изменено

- В README и `docs/**/*.md` удалены ручные переносы обычных абзацев и пробелы в
  конце строк. Правило однострочного хранения абзацев закреплено в политике
  документации; содержание документов, таблиц и блоков кода не изменено.
- Раздел установки в README сокращён до публичного имени пакета, требования к
  Node.js и команды `npm install`.
