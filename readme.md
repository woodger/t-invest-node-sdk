...
Для версии `0.5.3` Git tag остается `0.5.3` по исторической схеме проекта, а GitHub Release может называться `v0.5.3`. Release notes берутся из одноименного раздела `CHANGELOG.md`. Annotated tag требует настроенные `git user.name` и `git user.email`.
