...
- статическое деление лимита между предполагаемым числом процессов — доступно через `createInMemoryUnaryLimiter({ quotaShare })`, но не считается полноценной координацией: оно ломается при изменении concurrency и неэффективно использует свободную квоту;
...
SDK не реализует межпроцессный limiter. Публичный port позволяет Consumer-у подключить coordinator без изменения таблицы квот SDK, но не задаёт его runtime. `quotaShare` остаётся ручным статическим распределением, а не скрытым механизмом обнаружения процессов.
