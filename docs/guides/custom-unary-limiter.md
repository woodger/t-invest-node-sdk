...
Если число независимых owners заранее известно, каждому limiter instance можно выделить фиксированную долю исходных квот:
...
`quotaShare` принимает конечное число в диапазоне `[0.2, 1]`; значение по умолчанию — `1`. Нижняя граница сохраняет не меньше одного permit в исходном окне для минимальной package-квоты `5` запросов в минуту. При доле меньше единицы limiter вычисляет `maxRequests * quotaShare` отдельно для каждой переданной квоты. Результат от единицы и выше округляется вниз: например, `600` запросов за `60_000` мс превращаются в `300`, а `15` запросов за `1_000` мс — в `7`. Если Consumer override даёт значение меньше единицы, limiter сохраняет его как дробную скорость и увеличивает интервал между целыми permits: `1 * 0.2` означает один permit каждые `300_000` мс. Поэтому низкая пользовательская квота не приводит к ошибке во время RPC.

Эта настройка не меняет `TInvestUnaryLimitContext`: custom limiter по-прежнему получает исходные `maxRequests` и `windowMs` T-Invest. Она также не обнаруживает другие процессы и не синхронизирует их. При двух независимо настроенных процессах значение `0.5` делит известные квоты поровну; один оставшийся процесс использует только половину, а третий процесс нарушает сделанное предположение. Если несколько SDK instances используют один общий объект limiter-а, они уже делят его counters и дополнительное уменьшение доли для них не требуется.
