...
`quotaShare` принимает конечное число в диапазоне `(0, 1]`; значение по умолчанию — `1`. При доле меньше единицы limiter вычисляет `Math.floor(maxRequests * quotaShare)` отдельно для каждой переданной квоты. Например, `600` запросов за `60_000` мс превращаются в `300`, а `15` запросов за `1_000` мс — в `7`. Округление вниз не позволяет случайно превысить выделенную долю. Если результат меньше одного permit, `acquire()` отклоняет вызов с `RangeError` вместо скрытого повышения лимита до единицы.
