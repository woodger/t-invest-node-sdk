...
Отчёты CLI ветки `0.4.x` сохраняют `klong` и `kshort` без изменения значений для
совместимости существующего вывода. Написанные вручную контракты отчётов
помечают эти поля как устаревшие; их удаление потребует отдельного
несовместимого изменения.
