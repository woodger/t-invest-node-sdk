...
- использует сервисные fallback и более специфичные лимиты по полным gRPC
  method paths;
- ведет независимый график для каждого разрешенного service fallback или method
  override: методы, разрешившиеся в service fallback, делят общий график, а
  более специфичные method rules и другие сервисы не задерживают друг друга;
...
его графике одновременно. Это защитный локальный limiter, а не полная модель
всех агрегированных ограничений provider-а или IP.
