...
- CLI entrypoint в `src/bootstrap/bin/cli.ts`, CLI mechanics в `src/bootstrap/cli/**`, reusable CLI guards в `src/bootstrap/args`, registered handlers в `src/bootstrap/commands`, если они связаны через `package.json` `bin`;
