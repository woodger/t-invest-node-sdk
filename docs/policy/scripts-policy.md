...
Исключение: `test` script может передавать штатный флаг test runner-а `fwa --prune`, потому что он точечно удаляет только stale compiled tests без source-пары в рамках проверки test pipeline. Это не является заменой `clean` script и не должно превращаться в общую очистку `dist`.

Если после перемещения файлов в `dist` остались stale artifacts, их нужно удалять точечно и явно в рамках конкретной операции, а не менять постоянный build/test pipeline проекта.
