...
(0, node_test_1.describe)('bootstrap cli', () => {
    ...
    (0, node_test_1.describe)('runCli', () => {
        (0, node_test_1.test)('prints help when no command is provided', () => {
            const { io, read } = createIo();
            const exitCode = (0, cli_1.runCli)([], io);
        ...
        (0, node_test_1.test)('prints top-level help from help command and global flags', () => {
            for (const command of ['--help', '-h', 'help']) {
                const { io, read } = createIo();
                const exitCode = (0, cli_1.runCli)([command], io);
                node_assert_1.default.equal(exitCode, 0);
                node_assert_1.default.match(read().stdout, /Commands:/);
                node_assert_1.default.equal(read().stderr, '');
            }
        });
        (0, node_test_1.test)('prints command-specific help from a command help flag', () => {
            const { io, read } = createIo();
            const exitCode = (0, cli_1.runCli)(['version', '--help'], io);
            node_assert_1.default.equal(exitCode, 0);
        ...
        (0, node_test_1.test)('prints version from version command and global flag', () => {
        ...
        (0, node_test_1.test)('returns a failure for unknown commands', () => {
            const { io, read } = createIo();
            const exitCode = (0, cli_1.runCli)(['accounts'], io);
